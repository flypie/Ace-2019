...
L009B:  
	LD		SP,(RAMTOP)				; set stack-pointer to RAMTOP.
...
; left in the dictionary, prints 'ERROR' and the byte from address ERR_NO
...
L00EC:  CALL	L04B9					; forth
	...
	DEFW	ERR_NO					; -> system variable ERR_NO
...
; a carriage return to input buffer i.e. lower screen memory.
	...
	LD		(ENDSUF),HL				; set ENDBUF - end of logical line
	LD		(CURSOR),HL				; set the CURSOR
	...
	LD		(INSCRN),HL				; set INSCRN to this position.
	RET								; return.
...
L0198:  EX		AF,AF'					; preserve the output character.
	...
	LD		HL,(ENDSUF)				; fetch ENDBUF end of logical line
...
L01A6:  LD		DE,(L_HALF)				; fetch start of buffer from L_HALF
	LD		HL,0DBA0h				; 00000h - 02460h
	ADD		HL,DE					;
	JR		NC,L01E4				; forward to exit if buffer full.


	LD		HL,(SCRPOS)				; fetch position SCRPOS for upper screen
...
; reduced by 32 bytes a screen line.
	...
	LD		HL,INSCRN				; address INSCRN the left-hand location
...
L01FE:  LD		HL,STATIN				; system variable STATIN
...
L0211:  LD		HL,(CURSOR)				; fetch CURSOR position
	INC		HL						; and increment it.

	LD		DE,(ENDSUF)				; fetch ENDBUF - end of current line.
...
L0225:  LD		HL,(CURSOR)				; fetch CURSOR
...
L022C:  CALL	L0302					; routine finds characters to EOL.
	...
	LD		(CURSOR),DE				; else update CURSOR
...
L0254:  LD		HL,(INSCRN)				; fetch INSCRN start of current line.
	LD		DE,(L_HALF)				; fetch L_HALF start of buffer.
	...
	LD		HL,(INSCRN)				; fetch INSCRN leftmost location of
					; current line.
...
L02B0:  LD		HL,02700h				; this location is always zero.
	...
	LD		DE,(INSCRN)				; INSCRN		e.g. 026E0h
...
L02D8:  LD		HL,02700h				; one past end of screen.
	LD		DE,(L_HALF)				; fetch start of buffer from L_HALF
...
; -> called by retype
L02EA:  LD		HL,(L_HALF)				; fetch L_HALF

; -> from cursor down
L02ED:  LD		(INSCRN),HL				; set INSCRN
...
L0302:  LD		HL,(ENDSUF)				; system variable ENDBUF
	LD		DE,(CURSOR)				; system variable CURSOR
	AND		A						; prepare to subtract.
...
L0310:  CALL	L0336					; routine KEY_SCAN
	...
	LD		HL,(KEYCOD)				; load L with KEYCOD - last key pressed
...
L0332:  LD		(KEYCOD),HL				;  update KEYCOD/KEYCNT
...
L03F5:  LD		B,A						; save the character in the B register.
	...
	LD		HL,(EXWRCH)				; fetch possible vector from EXWRCH
					; (normally 0)
	LD		A,H						; test for
	OR		L						; the value zero.
...
L041C:  LD		(SCRPOS),HL				; update SCRPOS
...
L0421:  PUSH	AF						; save character
	...
	LD		HL,SCRPOS				; address the low order byte SCRPOS
...
; now calculate the number of characters to scroll in the upper display.
	...
	LD		HL,(L_HALF)				; fetch L_HALF the start of input buffer
...
L0462:  LD		DE,(STKBOT)				; system variable STKBOT.
	RST		10H						; push word DE
...
L0482:  DEFB	031h						; a single parameter low-byte of CURRENT.
...
L048F:  DEFB	03Eh						; low-order byte of FLAGS FLAGS
...
L0492:  DEFB	039h						; low-order byte of DICT DICT
...
L04AA:  LD		HL,FLAGS				; address FLAGS
...
L0538:  LD		A,(FLAGS)				; fetch system variable FLAGS
...
; ->
L0594:  LD		HL,STATIN				; fetch STATIN
	SET		0,(HL)					;
...
;
L05E1:  LD		HL,(L_HALF)				; fetch L_HALF - start of screen buffer.
	LD		(INSCRN),HL				; make INSCRN start of logical line the
					; same.
...
L0600:  PUSH	DE						; save delimiter
	...
	LD		DE,(L_HALF)				; else set DE from L_HALF
	CALL	L07FA					; routine SPACE_FILL (DE-HL)
	LD		(L_HALF),HL				; set L_HALF to next line
...
L0644:  LD		HL,(CONTEXT)				; fetch value of system variable CONTEXT
...
L084E:  LD		HL,(SPARE)				; fetch SPARE - start of Spare Memory.
	...
	LD		(SPARE),HL				; update the system variable SPARE to
...
L098F:  LD		HL,027FFh				; end of pad
	LD		(HLD),HL				; update system variable HLD
	JP		(IY)					; to 'next'.
...
L0AB1:  LD		HL,(SPARE)				; set pointer from system variable SPARE
...
L0B1B:  RST		18H						; pop word DE
	...
	LD		(SCRPOS),HL				; update system variable SCRPOS
...
L0B28:  ADD		A,020h
	...
	LD		E,A
	ADD		HL,DE
	LD		DE,(L_HALF)				; fetch start of lower half from L_HALF
	SBC		HL,DE
	ADD		HL,DE
	RET		C
...
L0F1D:  INC		DE						; increase source
	...
	LD		(DICT),HL				; store this location in SPARE
...
L0F36:  LD		HL,(STKBOT)				; fetch STKBOT
	LD		DE,(DICT)				; fetch SPARE
	...
	LD		H,A						; make HL zero
	LD		L,A						;
	LD		(DICT),HL				; update system variable SPARE to zero
...
; now increase the two data stack pointers.
	...
	LD		HL,(STKBOT)				; fetch value of STKBOT
	ADD		HL,BC					; add required room.
	LD		(STKBOT),HL				; update STKBOT.

	LD		HL,(SPARE)				; fetch value of SPARE
...
L11AD:  LD		HL,(CONTEXT)				; CONTEXT
	LD		(CURRENT),HL				; CURRENT
	JP		(IY)					; to 'next'.

; ---

L11B5:  LD		(CONTEXT),DE				; CONTEXT
	JP		(IY)					; to 'next'.
...
L14DC:  LD		HL,(STKBOT)				; fetch STKBOT
	AND		A						; clear carry flag
	SBC		HL,BC					; subtract number of bytes to reclaim.
	LD		(STKBOT),HL				; update STKBOT

	LD		HL,(SPARE)				; fetch SPARE
	SBC		HL,BC					; subtract number of bytes to reclaim.
	LD		(SPARE),HL				; update SPARE
...
L163A:  LD		HL,(CURRENT)				; CURRENT
	LD		DE,(CONTEXT)				; CONTEXT
	AND		A
	SBC		HL,DE
...
;
	...
	LD		HL,(SPARE)				; fetch end of data stack+1 from SPARE.
...
; update system variable SPARE - this could be deferred.
	...
	LD		(SPARE),HL				; update system variable SPARE
...
;L1D24:  LD		HL,(SPARE)				; fetch value from SPARE.
L1D24:  LD		HL,(SPARE)				; fetch value from SPARE.
